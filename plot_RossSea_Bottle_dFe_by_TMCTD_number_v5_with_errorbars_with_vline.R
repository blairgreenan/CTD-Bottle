...
#  ctd_bottle[[j]]@data$pressure[which(is.nan(ctd_bottle[[j]]@data$pressure))] <- NA
  ...
  dFe_data <- rbind(dFe_data, data.frame(Pressure=Press_omit,dFe=dFe_omit,Facet_cast=paste0("TM", TM_cast[j]), WD=water_depth[j-20], Survey="Survey 1"))
...
dFe_data <- rbind(dFe_data, data.frame(Pressure=Press_omit2,dFe=dFe_omit2,Facet_cast=paste0("TM", TM_cast[32]), WD=water_depth[6], Survey="Survey 2"))
...
ggsave("RossBank_dFe_errorbars_merge2.png", width = 10, height = 8, units = c("cm"), dpi = 1200, bg = "white", scale = 1.5)
ggsave("RossBank_dFe_errorbars_merge2.pdf", width = 10, height = 8, units = c("cm"), dpi = 1200, bg = "white", scale = 1.5)
