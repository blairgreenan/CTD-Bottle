...
# that were carried out over the NS section across Survey 1 of Ross Bank.
#
# load libraries
library(oce)
library(R.matlab)
...
# Create a subset for the NS section over Ross Bank CTD casts 45-50
RB <- subset(ctd_section, 45 <= stationId & stationId <= 50)
# Grid the data in the subset using the oce SectionGrid function
RBgrid2 <- sectionGrid(RB, p=seq(0,1000,10))
...
tiff("CTD45-50_bottle.tiff", width=6, height=6, units='in', res=1200, compression = 'lzw')
par(mfrow=c(3,2))
